...
namespace  driver {
	...
	NTSTATUS device_control(PDEVICE_OBJECT device_object, PIRP irp) {
		...
		debug_print("[+] dvice control called.\n");
...
NTSTATUS driver_main(PDRIVER_OBJECT driver_object,	PUNICODE_STRING registry_path) {
	...
	RtlInitUnicodeString(&device_name, L"\\Driver\\first_driver");
	...
	if (status != STATUS_SUCCESS) {
		debug_print("[-] failed to create driver dvice.\n");
		return status;
	}
	debug_print("[+] driver device successfully created.\n");

	UNICODE_STRING symbolic_link = {};
	RtlInitUnicodeString(&symbolic_link, L"\\DosDevices\\first_driver");
	...
	debug_print("[+] established symbolic link successfully.\n");
	...
	ClearFlag(driver_object->Flags, DO_DEVICE_INITIALIZING);
	debug_print("[+] dvice intialized successfully.\n");

	return status;
}
NTSTATUS DriverEntry() {
	debug_print("[+] nigga balls!");

	UNICODE_STRING driver_name = {};
	RtlInitUnicodeString(&driver_name, L"\\Driver\\first_driver");
